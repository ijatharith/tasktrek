...
<head>
    ...
    <title>Edit Assignment - TaskTrek</title>
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <main class="flex-1 flex flex-col overflow-hidden">
            <header class="bg-white shadow-sm h-20 flex items-center justify-between px-8 z-10">
                <div>
                    <h1 class="text-2xl font-bold text-black">Edit Assignment</h1>
                    <p class="text-gray-500 text-sm mt-1">Update details or mark assignment as completed</p>
                </div>
            </header>
            ...
            <div class="flex-1 overflow-y-auto p-6 md:p-8">
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden w-full min-h-[calc(100%-80px)] flex flex-col">
                    ...
                    <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="p-8 flex flex-col flex-1 h-full">
                        @csrf 
                        @method('PUT')
                        
                        <div class="mb-6">
                            <label for="title" class="block text-sm font-bold text-black mb-2">Task Title <span class="text-red-500">*</span></label>
                            <input type="text" id="title" name="title" value="{{ old('title', $task->title) }}" placeholder="e.g. Software Engineering Report" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                ...
                                <input type="text" id="course" name="course" value="{{ old('course', $task->course) }}" placeholder="e.g. BICS 2306" 
                                ...
                                <div class="flex space-x-3">
                                    <div class="relative">
                                        <input type="radio" name="priority" id="low" value="low"
                                            {{ strtolower(old('priority', $task->priority)) == 'low' ? 'checked' : '' }} class="peer sr-only">  
                                    ...
                                    <div class="relative">
                                        <input type="radio" name="priority" id="medium" value="medium"
                                            {{ strtolower(old('priority', $task->priority)) == 'medium' ? 'checked' : '' }} class="peer sr-only">
                                    ...
                                    <div class="relative">
                                        <input type="radio" name="priority" id="high" value="high"
                                            {{ strtolower(old('priority', $task->priority)) == 'high' ? 'checked' : '' }} class="peer sr-only">
                                ...
                                <div class="flex space-x-3">
                                    <div class="relative">
                                        <input type="radio" name="status" id="pending" value="Pending"
                                            {{ strtolower(old('status', $task->status)) == 'pending' ? 'checked' : '' }} class="peer sr-only">
                                        <label for="pending" class="flex items-center px-4 py-2 bg-[#E8EEF5] rounded-lg cursor-pointer hover:bg-amber-100 transition select-none peer-checked:bg-amber-100 peer-checked:ring-2 peer-checked:ring-amber-400">
                                    ...
                                    <div class="relative">
                                        <input type="radio" name="status" id="completed" value="Completed" 
                                            {{ strtolower(old('status', $task->status)) == 'completed' ? 'checked' : '' }} class="peer sr-only">
                                        <label for="completed" class="flex items-center px-4 py-2 bg-[#E8EEF5] rounded-lg cursor-pointer hover:bg-green-100 transition select-none peer-checked:bg-green-100 peer-checked:ring-2 peer-checked:ring-green-400">
                                            <span class="text-lg mr-2">✅</span>
                        ...
                        <div class="mb-8 flex-1 flex flex-col">
                            ...
                            <textarea id="description" name="description" placeholder="Add any details about this task..." 
                                class="w-full flex-1 min-h-[150px] bg-[#E8EEF5] text-gray-700 border-none rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors placeholder-slate-500 font-medium resize-none outline-none">{{ old('description', $task->description) }}</textarea>
                        </div>

                        <div class="flex items-center justify-between pt-4 border-t border-gray-100 bg-[#E8EEF5] -mx-8 -mb-8 px-8 py-4 mt-auto">
                            
                            <span class="text-xs text-gray-400 font-medium">Last updated {{ $task->updated_at->diffForHumans() }}</span>
